...
- **Recommendations → Google Sheets** (same Apps Script deployment as RSVP). `components/Recommendations.tsx` + `lib/recommendations.ts` call the same `/exec` URL (GET `action=listRecommendations`; POST `type: 'recommendation'` / `type: 'reply'`, `mode: 'no-cors'`). `Code.gs` auto-creates "Recommendations" (`id, city, title, body, author_name, created_at`) and "Replies" (`id, recommendation_id, body, author_name, created_at`) tabs on first submission. No env vars needed beyond the shared `NEXT_PUBLIC_GOOGLE_SCRIPT_URL`. Moderation = edit/delete rows in the Sheet. Chosen over Supabase specifically because Google Sheets doesn't suspend a project for inactivity.
